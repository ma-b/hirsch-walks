...
# (or by running Julia with the command-line option --code-coverage=FILE)
...
usage()
{
    ...
    echo "  -v             Verbose mode: list all uncovered lines in the output."
    ...
    echo "  -p             Use this option if paths in FILE are Windows-style paths."
}

while getopts ":hd:vp" option; do  # leading : (as in ":hd:v") skips errors (handled by user)
    case "${option}" in
        v)  verbose=1;;
        d)  # directory filter
            directories=(${OPTARG//,/ })  # split at commas and convert to array
            # check whether each directory in the list is a valid directory
        ...
        p)  # convert Windows-style paths in LCOV input to Unix-style paths
            windows_paths=1;;
        h)  # display usage
            usage
            exit 0;;
...
# replace \ with / and drive C: with /c
function convert_windows_path()
{
    filename=$(echo "$1" | sed 's/\\/\//g')
    for drive in C D; do
        # convert drive letters to lowercase
        filename=$(echo "$filename" | sed "s/${drive}:/\/\L${drive}/g")
    done
    echo $filename
}
...
# calculate and pretty-print coverage in percent,
# given the number of covered lines ($1) and the total line count ($2)
...
#----------------------------------------------------------------
# Parse input file (single pass) and print summary line by line.
# Implementation strategy: finite-state automaton with some
# additional memory for book-keeping.
#----------------------------------------------------------------

# these will count the total number of (covered) lines
ncovered_total=0
nlines_total=0
...
while read -r line; do
    if [[ $line == "end_of_record"* ]]; then  # end of current file
        ...
        if [[ $prefix == "SF" ]]; then
            if [[ ${#directories[@]} -eq 0 ]]; then
                ...
                if [[ -f $data ]]; then  # ignore filenames that are invalid (relative*/absolute) paths
                    ...
                    # convert file path to Unix path if necessary
                    if [[ $windows_paths -eq 1 ]]; then 
                        filename=$(convert_windows_path $data)
                    else
                        filename=$data
                    fi
                    
                    for directory in "${directories[@]}"; do
                        if [[ "$(realpath $filename)" == "$(realpath $directory)"* ]]; then
        ...
        # coverage count for a single code line
            ...
            # split line data at comma(s) and convert to an array (assuming a well-formed input file)
            arrdata=(${data//,/ })
            ...
            if [[ $execution_count -eq 0 ]]; then  # line was never executed
                ...
                elif [[ $line_number -gt $((prev_uncovered_line + 1)) ]]; then  
                    # current range cannot be extended to current line,
                    # so print end of range and open new range
                    echo -n "-${prev_uncovered_line}, $line_number"
                    isrange=0
                fi

                prev_uncovered_line=$line_number
            else  # line was executed at least once
                if [[ $isrange -eq 1 ]]; then
                    # print end of range
                    echo -n "-${prev_uncovered_line}"

                    # lines are listed consecutively, which means that
                    # a covered line closes any open range of uncovered lines
                    isrange=0
                fi
            fi
        ...
        # end of individual line coverage data
            ...
            # store number of covered lines in current file for LF line (next)
            ncovered=$data
...
# TODO print the following only if >0 lines
